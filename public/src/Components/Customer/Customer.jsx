...
const Customer = () => {
  ...
  function handlenavigate() {
    ...
    if (currentPath === "/customer/carbuyers") {
      navigate(`${currentPath}/generatepdf`);
    } else if (currentPath === "/customer/purchaseanalysis") {
      navigate(`${currentPath}/generatepdf`);
    } else if (currentPath === "/customer/salesforecast") {
      navigate(`${currentPath}/generatepdf`);
    } else if (currentPath === "/customer/onlineshoppers") {
      navigate(`${currentPath}/generatepdf`);
    }
  }
    ...
    if (file) {
      if (!file.path) {
        ...
        try {
          ...
          const response = await axios.post(
            `http://127.0.0.1:5001/generate-report`,
            formData,
            {
              headers: {
                "Content-Type": "multipart/form-data",
  ...
  const handleDeleteFile = async (filename) => {
    try {
      const response = await axios.delete(
        `${path}files/del/${filename}/${auth().data._id}`
      );
      if (response) {
